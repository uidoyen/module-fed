...
exports.DrawerPanelContent = (_a) => {
    var { className = '', id, children, hasNoBorder = false, isResizable = false, onResize, minSize, defaultSize, maxSize, increment = 5, resizeAriaLabel = 'Resize', resizeAriaDescribedBy = 'Press space to begin resizing, and use the arrow keys to grow or shrink the panel. Press enter or escape to finish resizing.', widths } = _a, props = tslib_1.__rest(_a, ["className", "id", "children", "hasNoBorder", "isResizable", "onResize", "minSize", "defaultSize", "maxSize", "increment", "resizeAriaLabel", "resizeAriaDescribedBy", "widths"]);
    const panel = React.useRef();
    const [panelIsClosed, setPanelIsClosed] = React.useState(false);
    const { position, isExpanded, isStatic, onExpand, drawerRef } = React.useContext(Drawer_1.DrawerContext);
    let currWidth = 0;
    let panelRect;
    let right;
    let left;
    ...
    return (React.createElement("div", Object.assign({ id: id, className: react_styles_1.css(drawer_1.default.drawerPanel, isResizable && drawer_1.default.modifiers.resizable, hasNoBorder && drawer_1.default.modifiers.noBorder, util_1.formatBreakpointMods(widths, drawer_1.default), className), ref: panel, onTransitionEnd: ev => {
